...
function Sidebar({sidebarOpen, setSidebarOpen}: any) {
  ...
  const storedSidebarExpanded = localStorage.getItem("sidebar-expanded") as any;
  const [sidebarExpanded, setSidebarExpanded] = useState(storedSidebarExpanded === null ? false : storedSidebarExpanded === "true") as any;
      ...
      >
        ...
        <div className='flex justify-between mb-10 pr-3 sm:px-2'>
          ...
          <NavLink end to='/' className='block'>
            <div>
              <h1>
                <span className='text-2xl md:text-3xl text-gray-100 font-bold mb-1 hover:text-gray-400 transition-colors duration-200'>CMD</span>
              </h1>
            </div>
          </NavLink>
        </div>
        ...
        <div className='space-y-8'>
          ...
          <div>
            ...
            <ul className='mt-3'>
              ...
              {/* Bank Statement */}
              <SidebarLinkGroup activecondition={pathname === "/BankStatement" || pathname.includes("BankStatement")}>
                {(handleClick: any, open: any) => {
                  return (
                    <React.Fragment>
                      <a
                      ...
                      >
                        <div className='flex items-center justify-between'>
                          <div className='flex items-center'>
                            <svg className='shrink-0 h-6 w-6' viewBox='0 0 24 24'>
                              <path
                                className={`fill-current text-slate-600 ${pathname.includes("BankStatement") && "text-indigo-500"}`}
                                d='M18.974 8H22a2 2 0 012 2v6h-2v5a1 1 0 01-1 1h-2a1 1 0 01-1-1v-5h-2v-6a2 2 0 012-2h.974zM20 7a2 2 0 11-.001-3.999A2 2 0 0120 7zM2.974 8H6a2 2 0 012 2v6H6v5a1 1 0 01-1 1H3a1 1 0 01-1-1v-5H0v-6a2 2 0 012-2h.974zM4 7a2 2 0 11-.001-3.999A2 2 0 014 7z'
                              />
                              <path
                                className={`fill-current text-slate-400 ${pathname.includes("BankStatement") && "text-indigo-300"}`}
                        ...
                        <ul className={`pl-9 mt-1 ${!open && "hidden"}`}>
                          <li className='mb-1 last:mb-0'>
                            <NavLink
                              ...
                              className={({isActive}) => "block text-slate-400 hover:text-slate-200 transition duration-150 truncate " + (isActive ? "!text-indigo-500" : "")}
                            >
                              <span className='text-sm font-medium lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200'>Download Statement</span>
                            </NavLink>
                          </li>
                        </ul>
                      </div>
